...
betsNumber = int(input(f'Informe quantas apostas você deseja fazer: ').strip())
print()

for betCount in range(1, betsNumber + 1):
    bet.clear()
    bet = sample(range(1, 61),6)
    bet.sort()
    sleep(0.5)
    print(f'======= {colors["backGreen"]} Aposta {betCount} {colors["clean"]} =======')
    print(f' {colors["blue"]}{bet}{colors["clean"]} ')
